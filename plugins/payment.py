...
# --- 1. PAYMENT SELECTION (Plan select karne ke baad) ---
...
def handle_selection(call):
    try:
        ...
        if not data:
            return bot.send_message(call.message.chat.id, "❌ Database error: Item not found.")

        # Price calculation
        price = data.get('price') if 'story_name' in data else data.get('plans', {}).get(mins, "0")
        ...
        # Photo card ko delete karke fresh message bhejna zyada smooth hota hai
...
def send_request_to_admin(message, item_id, mins):
    if message.content_type != 'photo':
        msg = bot.send_message(message.chat.id, "❌ Please sirf Screenshot bhejein!")
        bot.register_next_step_handler(msg, send_request_to_admin, item_id, mins)
        return
    ...
    markup.add(InlineKeyboardButton("💬 ᴄᴏɴᴛᴀᴄᴛ ᴜsᴇʀ", url=f"tg://user?id={message.from_user.id}"))
...
def admin_approve(call):
    ...
    # Expiry calculation (manual = Lifetime/3650 days)
    expiry = int(time.time()) + (int(mins) * 60) if mins != 'manual' else int(time.time()) + (3650*24*60*60)
    users_col.update_one({"user_id": int(u_id), "channel_id": data.get('channel_id', item_id)}, 
                         {"$set": {"expiry": expiry}}, upsert=True)
...
def admin_reject(call):
    ...
    bot.send_message(u_id, "❌ Aapka payment reject ho gaya hai. Support se sampark karein.")
